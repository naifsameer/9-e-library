let VEditPublisher = Vue.defineComponent({
  template: html`
    <div>
      <button class="border rounded  px-4 py-2 mb-5" @click="$router.go(-1)">
        Back
      </button>
      <h4 class="text-2xl mb-6">Edit publisher info</h4>
    </div>
    ...
    <form ref="editFormRef" @submit.prevent="onSubmit" class="space-y-4">
      ...
      <v-input name="address" label="address"></v-input>

      <v-input name="country" label="country"></v-input>
  ...
  methods: {
    onSubmit() {
      ...
      axios.post('/publishers/edit', formData).then((res) => {
        console.log(res);
      });
    },
  },
