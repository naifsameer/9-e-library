let VCities = Vue.defineComponent({
  template: html`
    ...
    <v-modal :show="isAddModalOpen" :onClose="closeAddModal">
      <h4 class="text-2xl mb-6">Add new City</h4>

      <form @submit.prevent="onAddFormSubmit">
        <v-input v-model="name" name="name" label="name"></v-input>
    ...
    <v-modal :show="isEditModalOpen" :onClose="closeEditModal">
      <h4 class="text-2xl mb-6">Edit City</h4>
    ...
    <v-modal :show="isDeleteModalOpen" :onClose="closeDeleteModal">
      <h4 class="text-2xl mb-6">Are you sure you want to delete this city</h4>
  ...
  methods: {
    ...
    onEditFormSubmit() {
      this.formData.append('name', this.name);
      this.formData.append('city_id', this.city_id);

      axios
        .post('/cities/update', this.formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        })
    ...
    onAddFormSubmit() {
      axios.post('/cities', this.city_id).then((res) => {
        this.resetData();
      });
    },
